...
public class Grid {
	...
	public boolean isCellUnavailable(Cell oldCell, Cell newCell) {
		...
		if (reservationTable.containsKey(newCell)) {
			if (debug)
				System.out.println(newCell.toString() + " DENIED");
			return true;
		}

		if (debug)
			System.out.println(newCell.toString() + " NOT RESERVED");
		...
		if (isNextPosReservedLastStep) {
			...
			if (debug)
				System.out.println(newCell.toString() + " RESERVED LAST STEP");
			boolean isLastPosReservedNextStep = isCellReserved(new Cell(oldCell.getXY(), newCell.getStep()));

			if (isLastPosReservedNextStep) {
				if (debug)
					System.out.println(newCell.toString() + " LAST POS RESERVED NEXT STEP");
				boolean identicalIds = reservationTable.get(new Cell(oldCell.getXY(), newCell.getStep())) == r1;

				if (debug && identicalIds)
					System.out.println(newCell.toString() + " identicalIds");
				else if (debug)
					System.out.println(newCell.toString() + " not identicalIds");
				return identicalIds;
			} else
				return false;
		} else {
			if (debug)
				System.out.println(newCell.toString() + " IS AVAILABLE");
			return false;
		}
	}
		...
		for (int y = -1; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
				if (y == -1) {
					...
					if (markerPos.equals(new Point(x, y)))
						System.out.print("[" + marker + "]");
					else if (obstacles.contains(new Point(x, y)))
						System.out.print("[ # ]");
					else if (items.contains(new Point(x, y)))
						System.out.print("[ I ]");
					else
			...
			for (int x = 0; x < width; x++) {
				if (y == -1) {
					...
					if (markerPos.equals(new Point(x, y)))
						...
						Optional<Integer> prio = Optional
								.ofNullable(reservationTable.get(new Cell(new Point(x, y), step)));
