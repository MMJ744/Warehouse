...
public class RouteIntegration {
	/*
	 * read in jobs from job allocation is this running as a thread? set up grid
	 * once at the start
	 * 
	 */
	...
	public Route planRoute(Robot r, int s) throws PathNotFoundException {
		...
		Point dropPoint = findClosestDropOff(
				new Point(orderedParts[orderedParts.length - 1].getX(), orderedParts[orderedParts.length - 1].getY()));
		// death statement just gets the x & y coordinates of the final job part to be
		// conducted
		...
		for (int i = 0; i <= orderedParts.length; i++) {
			...
			for (int j = 0; j < path.size(); j++) {
				...
				if (action.equals(ACTION.PICKUP))
					route.addToRoute(new RouteAction(action, point, robot.getID(), itemID, itemCount));
				else
	...
	private static JobPart[] orderParts(JobPart[] unordered, Point start) {
		...
		for (int i = 0; i < ordered.length; i++) {
			...
			for (int j = 0; j < unordered.length; j++) {
				int manDist = Math.abs(startPoint.x - unordered[j].getX())
						+ Math.abs(startPoint.y - unordered[j].getY());
				if (manDist < compDist) {
