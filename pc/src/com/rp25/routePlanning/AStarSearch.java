...
public class AStarSearch {
	...
	public List<Point> getPath(Robot r, Point startXY, Point goalXY, int priority, int startStep)
	...
	public List<Point> getFullPath(Robot r, Point startXY, Collection<Point> itemXYs, Point goalXY, int priority)
			throws PathNotFoundException {
	...
	public Route getFullRoute(RouteDesc desc) throws PathNotFoundException {
		...
		if (items.hasNext()) {
			...
			for (Point point : path) {
				ACTION a = (point.equals(item) && !hasPickedUp) ? ACTION.PICKUP
						: (point.equals(lastPoint) ? ACTION.WAIT : ACTION.MOVE);
				if (a.equals(ACTION.PICKUP))
					hasPickedUp = true;
				lastPoint = point;
	...
	public static void warehouseTest() {
		Point[] obstacleArray = new Point[] { new Point(1, 2), new Point(1, 3), new Point(1, 4), new Point(1, 5),
				new Point(1, 6), new Point(4, 2), new Point(4, 3), new Point(4, 4), new Point(4, 5), new Point(4, 6),
				new Point(7, 2), new Point(7, 3), new Point(7, 4), new Point(7, 5), new Point(7, 6), new Point(10, 2),
	...
	private class AStar {
		...
		public List<Point> getPath(boolean debug) throws PathNotFoundException {
			...
			while (!openNodes.isEmpty()) {
				...
				if (searchNode.getF() != Integer.MAX_VALUE && searchNode.getF() > 30)
					break;

				if (debug)
					System.out.println(new Cell(searchNode.getXY(), searchNode.getStep()).toString()
							+ " checked with F: " + searchNode.getF());
			...
			throw new PathNotFoundException("Could not find a path starting at: " + startXY.toString()
					+ " and ending at: " + goalXY.toString());
		}
		...
		public List<Point> grabRoute(Node node) {
			...
			if (parent.isPresent())
				route.addAll(grabRoute(parent.get()));
