...
public class Cancellation {
	...
	public Cancellation(String cancellations, String testJobs, ArrayList<Item> items) {
		try {
			BufferedReader testsReader = new BufferedReader(
					new InputStreamReader(getClass().getResourceAsStream(testJobs)));
			BufferedReader cancellationReader = new BufferedReader(
					new InputStreamReader(getClass().getResourceAsStream(cancellations)));
			allItems = items;
			String line1;
			String line2;
			int linesRead = 0;
			while ((line1 = testsReader.readLine()) != null && (line2 = testsReader.readLine()) != null) {
				...
				allTestJobs.add(
						new TestJob(itemName, numOfItems, numOfTypes, reward, weight, Integer.parseInt(cancelInfo[1])));
	...
	public void learn() {
		...
		for (int i = trainingSetSize; i < allTestJobs.size(); i++) {
			...
			if ((thisCancellation == 1 && calculatedCancelltion.compareTo(new BigDecimal("0.5")) > -1)
					|| (thisCancellation == 0 && calculatedCancelltion.compareTo(new BigDecimal("0.5")) < 1)) {
				correctPredicts += 1;
			}
		}
		timesLearnt += 1;
	...
	public BigDecimal testProbOfCancellation(TestJob job) {
		...
		if (job.getNumOfItemTypes() - 1 < 0) {
			if (probGivenTypes[0] == 0) {
				...
				typesProb = new BigDecimal(probYesGivenTypes[0]).divide(new BigDecimal(probGivenTypes[0]), 5,
						RoundingMode.HALF_UP);
			}
		} else if (probYesGivenTypes[job.getNumOfItemTypes() - 1] == 0) {
			typesProb = new BigDecimal("0.001");
		} else {
			typesProb = new BigDecimal(probYesGivenTypes[job.getNumOfItemTypes() - 1])
					.divide(new BigDecimal(probGivenTypes[job.getNumOfItemTypes() - 1]), 5, RoundingMode.HALF_UP);
		}
		...
		if (numOfItems / 2 - 1 < 0) {
			if (probGivenItems[0] == 0) {
				...
				itemsProb = new BigDecimal(probYesGivenItems[0]).divide(new BigDecimal(probGivenItems[0]), 5,
						RoundingMode.HALF_UP);
			}
		} else if (probGivenItems[numOfItems / 2 - 1] == 0) {
			itemsProb = new BigDecimal("0.001");
		} else {
			itemsProb = new BigDecimal(probYesGivenItems[numOfItems / 2 - 1])
					.divide(new BigDecimal(probGivenItems[numOfItems / 2 - 1]), 5, RoundingMode.HALF_UP);
		}
		...
		if (weight.divide(new BigDecimal("10")).intValue() - 1 < 0) {
			if (probGivenWeight[0] == 0) {
				...
				weightProb = new BigDecimal(probYesGivenWeight[0]).divide(new BigDecimal(probGivenWeight[0]), 5,
						RoundingMode.HALF_UP);
			}
		} else if (probYesGivenWeight[weight.divide(new BigDecimal("10")).intValue() - 1] == 0) {
			weightProb = new BigDecimal("0.001");
		} else {
			weightProb = new BigDecimal(probYesGivenWeight[weight.divide(new BigDecimal("10")).intValue() - 1]).divide(
					new BigDecimal(probGivenWeight[weight.divide(new BigDecimal("10")).intValue() - 1]), 5,
					RoundingMode.HALF_UP);
		}
		...
		if (reward.divide(new BigDecimal("4")).intValue() - 1 < 0) {
			if (probGivenReward[0] == 0) {
				...
				rewardProb = new BigDecimal(probYesGivenReward[0]).divide(new BigDecimal(probGivenReward[0]), 5,
						RoundingMode.HALF_UP);
			}
		} else if (probYesGivenWeight[reward.divide(new BigDecimal("4")).intValue() - 1] == 0) {
			rewardProb = new BigDecimal("0.001");
		} else {
			rewardProb = new BigDecimal(probYesGivenReward[reward.divide(new BigDecimal("4")).intValue() - 1]).divide(
					new BigDecimal(probGivenTypes[reward.divide(new BigDecimal("4")).intValue() - 1]), 5,
					RoundingMode.HALF_UP);
		}
	...
	public BigDecimal probOfCancellation(Job job) {
		...
		if (parts.size() - 1 < 0) {
			if (probGivenTypes[0] == 0) {
				...
				typesProb = new BigDecimal(probYesGivenTypes[0]).divide(new BigDecimal(probGivenTypes[0]),
						RoundingMode.HALF_UP);
			}
		} else if (probGivenTypes[parts.size() - 1] == 0) {
			typesProb = new BigDecimal("0.0001");
		} else {
			typesProb = new BigDecimal(probYesGivenTypes[parts.size() - 1])
					.divide(new BigDecimal(probGivenTypes[parts.size() - 1]), RoundingMode.HALF_UP);
		}
		BigDecimal itemsProb;
		if (numOfItems / 2 - 1 < 0) {
			if (probGivenItems[0] == 0) {
				...
				itemsProb = new BigDecimal(probYesGivenItems[0]).divide(new BigDecimal(probGivenItems[0]),
						RoundingMode.HALF_UP);
			}
		} else if (probGivenItems[numOfItems / 2 - 1] == 0) {
			itemsProb = new BigDecimal("0.0001");
			...
			itemsProb = new BigDecimal(probYesGivenItems[numOfItems / 2 - 1])
					.divide(new BigDecimal(probGivenItems[numOfItems / 2 - 1]), RoundingMode.HALF_UP);
		}
		BigDecimal weightProb;
		if (weight.divide(new BigDecimal("10")).intValue() - 1 < 0) {
			if (probGivenWeight[0] == 0) {
				...
				weightProb = new BigDecimal(probYesGivenWeight[0]).divide(new BigDecimal(probGivenWeight[0]),
						RoundingMode.HALF_UP);
			}
			...
			weightProb = new BigDecimal(probYesGivenWeight[weight.divide(new BigDecimal("10")).intValue() - 1]).divide(
					new BigDecimal(probGivenWeight[weight.divide(new BigDecimal("10")).intValue() - 1]),
					RoundingMode.HALF_UP);
		}
		BigDecimal rewardProb;
		if (reward.divide(new BigDecimal("4")).intValue() - 1 < 0) {
			if (probGivenReward[0] == 0) {
				...
				rewardProb = new BigDecimal(probYesGivenReward[0]).divide(new BigDecimal(probGivenReward[0]),
						RoundingMode.HALF_UP);
			}
			...
			rewardProb = new BigDecimal(probYesGivenReward[reward.divide(new BigDecimal("4")).intValue() - 1]).divide(
					new BigDecimal(probGivenReward[reward.divide(new BigDecimal("4")).intValue() - 1]),
					RoundingMode.HALF_UP);
		}
		probOfCancellation = probYes.multiply(typesProb).multiply(itemsProb).multiply(weightProb).multiply(rewardProb);
		return probOfCancellation;
	...
	private class TestJob {
		...
		public TestJob(String name, int numOfItems, int numOfItemTypes, BigDecimal reward, BigDecimal weight,
				int isCancelled) {
			this.name = name;
