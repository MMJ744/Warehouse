...
public class JobSelection {
	...
	public JobSelection(String jobLocation, String itemsLocation, String locationLocation, String cancelLocation,
			String testLocation) {
		try {
			...
			while ((line = brLocation.readLine()) != null) {
				...
				while ((line2 = brItems.readLine()) != null) {
					...
					if (itemParts[0].equals(locationParts[2])) {
						Item item = new Item(itemParts[0], Integer.parseInt(locationParts[0]),
								Integer.parseInt(locationParts[1]), new BigDecimal(itemParts[1]),
								new BigDecimal(itemParts[2]));
						items.add(item);
					}
				}
			}
				...
				for (int i = 1; i < jobParts.length; i += 2) {
					for (Item item : items) {
						if (jobParts[i].equals(item.getName())) {
							job.addPart(new JobPart(item.getName(), item.getX(), item.getY(),
									Integer.parseInt(jobParts[i + 1]), item.getWeight(), item.getReward()));
						}
					}
				}
			}
	...
	public void calculatePriority() {
		for (Job job : allJobs) {
			...
			if (cancel.probOfCancellation(job).compareTo(new BigDecimal(0)) > 0) {
				priority = reward.divide(weight.add(new BigDecimal(numberOfPlaces)), 5, RoundingMode.HALF_UP)
						.divide(cancel.probOfCancellation(job), 5, RoundingMode.HALF_UP);
			} else {
				priority = reward.divide(weight.add(new BigDecimal(numberOfPlaces)), 5, RoundingMode.HALF_UP)
						.divide(new BigDecimal("0.001"), 5, RoundingMode.HALF_UP);
			}
