...
public class JobAllocation {
	...
	public static void allocateJob(Job nextJob) {
		...
		/*
		 * if(totalWeight.compareTo(new BigDecimal("50")) > 0) { jobIsSplit = true;
		...
		/*
		 * if(jobIsSplit) { if(Math.max(numRobot1Jobs, Math.max(numRobot2Jobs,
		 * numRobot3Jobs))==numRobot1Jobs) { robot1Jobs.addAll(splitJobs); } else
		 * if(Math.max(numRobot1Jobs, Math.max(numRobot2Jobs,
		 * numRobot3Jobs))==numRobot2Jobs) { robot2Jobs.addAll(splitJobs); } else {
		 * robot3Jobs.addAll(splitJobs); } } else {
		 */
